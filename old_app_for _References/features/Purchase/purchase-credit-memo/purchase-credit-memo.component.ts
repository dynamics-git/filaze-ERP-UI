...
import { PurchaseCreditMemoCalculation, PurchaseCreditMemoeHeader, PurchaseCreditMemoLine } from './purchase-credit-memo.config';
...
@Component({
  standalone: false,
  selector: 'app-purchase-credit-memo',
  template: `
...
export class PurchaseCreditMemoComponent {

  config: DataTableConfig = {
    title: 'Purchase Credit Memo',
    idProp: 'Id',
    headerApi: '/purchCrMemoHeaders',
    pageName: 'PCM',
    ...
    addItemConfig: {
      ...
      headerConfig: PurchaseCreditMemoeHeader,
      lineConfig: PurchaseCreditMemoLine,
      calculationSectionConfig: PurchaseCreditMemoCalculation,
  ...
  buttonClickEvent(buttonData: CustomButtonEvent) {
    ...
    if (buttonData.button.label === 'PostAsCM') {
      if (buttonData.data.VendorCrMemoNo) {
        this.addItemService.showLoader$.next(true);
        const url: string = '(' + buttonData.data[this.config.idProp!] + ')/Microsoft.NAV.postPurchCrMemo';
        this.restService.post(this.config.headerApi + url, {}).subscribe((response: any) => {
          this.addItemService.showLoader$.next(false);
        ...
        this.toastr.error('Fill Vendor Credit Memo No')
      }
    }
  }
