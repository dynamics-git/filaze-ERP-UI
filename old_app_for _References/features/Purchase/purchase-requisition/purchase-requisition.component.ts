...
import { PurchaseRequisitionCalculation, PurchaseRequisitionHeader, PurchaseRequisitionLine, PurchaseRequisitionListHeaders, RfqVendorItemConfig } from './purchase-requisition.config';
...
export class PurchaseRequisitionComponent implements OnInit {
  ...
  private openCombineProcureList(buttonData: CustomButtonEvent) {
    ...
    PurchaseRequisitionComponent.combineProcureHeaderData = targetHeader;

    const popup = this.universalPopupService.open(
      'purchaseRequisitionCombine',
      this
    );
    PurchaseRequisitionComponent.combineProcurePopupRef = popup;
  }


  private CombinedPR(buttonData: CustomButtonEvent) {

    const storedHeaderData = PurchaseRequisitionComponent.combineProcureHeaderData;
    ...
    this.restService.post(url, payload).subscribe({
      next: () => {
        ...
        PurchaseRequisitionComponent.combineProcurePopupRef?.close({
          action: 'CombinedPR',
          record: prNos
        });
