...
import { SalesCreditMemoecalculation, SalesCreditMemoeLine, SalesCreditMemoeHeader } from './sales-credit-memo.config';
...
@Component({
  standalone: false,
  selector: 'app-sales-credit-memo',
...
export class SalesCreditMemoComponent {

  config: DataTableConfig = {
    title: 'Sales Credit Memo',
    idProp: 'Id',
    headerApi: '/salesCeMemoHeaders',
    pageName: 'SCM',
    ...
    addItemConfig: {
      ...
      headerConfig: SalesCreditMemoeHeader,
      lineConfig: SalesCreditMemoeLine,
      calculationSectionConfig: SalesCreditMemoecalculation,
  ...
  buttonClickEvent(buttonData: CustomButtonEvent) {
    ...
    if (buttonData.button.label === 'PostAsCM') {
      ...
      this.addItemService.showLoader$.next(true);
      const url: string = '(' + buttonData.data[this.config.idProp!] + ')/Microsoft.NAV.postSalesCrMemo';
      this.restService.post(this.config.headerApi + url, {}).subscribe((response: any) => {
